...
import { designSystemCss, heroHeader, heroAside, themeBootstrapScript, agencyName } from "./lib/design_system.js";
...
function render(slug) {
  const raw = JSON.parse(readFileSync(P.clientFile(slug, "audit_raw.json"), "utf8"));
  const mdPath = P.clientFile(slug, "audit_report.md");
...
<footer>Generated by ${agencyName()} on ${date}. Baseline scope: ${paid.skipped || (paid.total_spend_lifetime ?? 0) === 0 ? "organic + account-readiness (no paid spend history)" : "full organic + paid"}. Immutable baseline for future before/after comparisons.</footer>
</div></body></html>`;
}
