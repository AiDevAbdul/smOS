...
- **Measurement is one spine:** `/capi-setup` writes Event Match Quality (per event, with match-key coverage) and modeled-vs-observed conversion reconciliations to `clients/<slug>/data/measurement_spine.json` (`scripts/lib/measurement_spine.js`); `/attribution` reads that record rather than re-deriving it. An EMQ Meta does not return is `null` (unknown), never 0; a trend needs two real samples; a coverage ratio with a zero or unknown denominator is `null` with the reason stated. Every conversion count carries its side — platform-reported (modeled) or audited. Offline or token-less, event status is `unknown`, never `never_fired` — that would assert something about a pixel smOS never queried.
...
- **Never let unreachable equal zero.** `/listening` reports one `coverage` row per
  source: a platform with no smOS-reachable listening API (TikTok, X, LinkedIn,
  YouTube, Reddit) is `unavailable` with `mentions: null`, and `0` means only "we
  looked and found nothing". Share of voice is computed over `basis_platforms` alone,
  carries a `confidence` share, and is labelled `is_floor` whenever a contributing
  source is partial or truncated. Crisis detection needs 5 prior time-series points
  before it may call a spike; below that it is `insufficient_data`, never a severity —
  and `elevated`/`critical` always `requires_human`. smOS never auto-responds to one.
...
- **Benchmarks are vertical + geo, and say how close they are.** `scripts/meta-ad-library/benchmarks.json` + `benchmarks.py resolve(vertical, geo)` back `/pre-audit` (`--vertical`, `--geo`). No public source holds an observed vertical-by-geo cell, so every metric carries a `basis`: `vertical_observed`, `geo_derived` (vertical × the market's CPM index — an estimate, badged as such), or `global_default`. Only CPM is geo-adjusted; there is no CPC/CPA index to adjust by. A market absent from the table (PK included) gets `cpm_index: null` and **no substitute country**; a vertical with no defensible proxy (nonprofit) falls back to the cross-vertical floor and says so. Never present a fallback or a derived figure as the prospect's measured category number.
