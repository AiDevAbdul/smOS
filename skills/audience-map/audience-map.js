 ...
 * produced from this by the shared design-system renderer (md_to_html.js) so the
 * targeting plan matches every other smOS report — never hand-roll CSS here.
 */
export function renderMarkdown(map, profile = {}) {
  const name = profile?.name || map.client_slug;
  const g = map.geo || {};
  const ag = map.age_gender || {};
  let md = `## Overview\n\n`;
  md += `Targeting plan for **${name}** — ${(map.clusters || []).length} interest clusters, `;
  md += `${(map.retargeting_layers || []).length} retargeting layers, ${(map.lookalikes || []).length} lookalikes.\n\n`;

  md += `## Geo & Demographics\n\n`;
  if (g.primary) {
    const p = g.primary;
    md += `- **Geo:** ${p.type === "radius" ? `${p.radius_miles}mi around ${p.center}` : JSON.stringify(p)}\n`;
  }
  md += `- **Age:** ${ag.age_min ?? "?"}–${ag.age_max ?? "?"} · **Gender:** ${(ag.genders || ["all"]).join(", ")}\n`;
  md += `- **Language:** ${(map.language || []).join(", ") || "—"}\n\n`;

  md += `## Interest Clusters\n\n`;
  md += `| Cluster | Objective | Income layer | Est. size | Interests |\n|---|---|---|---|---|\n`;
  for (const c of map.clusters || []) {
    md += `| ${c.label || c.id} | ${(c.objective_fit || []).join(", ")} | ${c.income_layer || "—"} | ${c.size_estimate || "—"} | ${(c.interest_stack || []).slice(0, 5).join(", ")} |\n`;
  }

  md += `\n## Retargeting Layers\n\n`;
  md += `| Layer | Source | Window | Min-size warning |\n|---|---|---|---|\n`;
  for (const r of map.retargeting_layers || []) {
    md += `| ${r.id} | ${r.source} | ${r.window_days ? r.window_days + "d" : "—"} | ${r.min_size_warning_at || "—"} |\n`;
  }

  md += `\n## Lookalikes\n\n`;
  md += `| Lookalike | Seed | Country | % |\n|---|---|---|---|\n`;
  for (const l of map.lookalikes || []) {
    md += `| ${l.id} | ${l.seed} | ${l.country || "—"} | ${l.pct != null ? l.pct + "%" : "—"} |\n`;
  }

  if ((map.exclusions || []).length) {
    md += `\n## Exclusions\n\n`;
    for (const e of map.exclusions) md += `- **${e.id}** — ${e.rule || e.from}\n`;
  }

  if ((map.ad_set_recommendations || []).length) {
    md += `\n## Recommended Ad Sets\n\n`;
    md += `| Ad set | Cluster | Objective | Daily budget |\n|---|---|---|---|\n`;
    for (const a of map.ad_set_recommendations) {
      md += `| ${a.name} | ${a.cluster} | ${a.objective} | $${a.daily_budget_low}–$${a.daily_budget_high} |\n`;
    }
  }
...
async function main() {
  ...
  const { htmlPath, pdfOk } = writeHtmlAndPdf(mdPath, md, {
    title: `${profile?.name || slug} — Audience Map`,
    subtitle: `${(map.clusters || []).length} clusters · ${(map.retargeting_layers || []).length} retargeting layers`,
    eyebrow: "smOS · Targeting",
  });
