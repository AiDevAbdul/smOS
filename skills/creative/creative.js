...
/**
 * Render ad copy as a client-facing markdown deliverable. Defensive across both
 * the lint output shape (hooks as objects with best_combo) and the older on-disk
 * shape (hooks as strings + headline/primary arrays with per-variant scores).
 * HTML+PDF come from the shared design-system renderer — never hand-roll CSS.
 */
export function renderMarkdown(adCopy, profile = {}) {
  const name = profile?.name || adCopy.client_slug;
  const angles = adCopy.angles || [];
  const topByScore = (arr, scoreKey) =>
    (arr || []).reduce((a, b) => {
      const bs = b?.[scoreKey]?.composite ?? b?.scores?.overall ?? -1;
      const as = a?.[scoreKey]?.composite ?? a?.scores?.overall ?? -1;
      return bs > as ? b : a;
    }, null);

  let md = `## Overview\n\n`;
  md += `Ad copy for **${name}** — ${angles.length} creative angles. `;
  if (adCopy.summary) md += `${adCopy.summary.total_hooks ?? "?"} hooks, ${adCopy.summary.compliance_violations ?? adCopy.summary.non_compliant ?? 0} compliance violations.`;
  md += `\n\n`;

  for (const a of angles) {
    const title = a.name || a.angle_id;
    const family = a.hook_family || a.hook_archetype || "";
    md += `## ${title}${family ? ` — _${family}_` : ""}\n\n`;

    // Hooks: string list (old) or objects with .hook.text (lint)
    const hookStrings = (a.hooks || [])
      .map((h) => (typeof h === "string" ? h : h?.hook?.text || h?.text))
      .filter(Boolean);
    if (hookStrings.length) {
      md += `**Hooks**\n`;
      for (const h of hookStrings) md += `- ${h}\n`;
      md += `\n`;
    }

    // Recommended combo
    let recHeadline, recPrimary, recCta;
    const tp = (a.hooks || []).find((h) => h?.top_pick);
    if (tp?.best_combo) {
      recHeadline = tp.best_combo.headline;
      recPrimary = tp.best_combo.primary;
      recCta = tp.best_combo.cta;
    } else {
      recHeadline = topByScore(a.headlines, "score")?.text;
      recPrimary = topByScore(a.primary_text, "score")?.text;
      recCta = Array.isArray(a.ctas) ? a.ctas[0] : undefined;
    }
    if (recHeadline || recPrimary || recCta) {
      md += `**Recommended combo**\n\n`;
      if (recHeadline) md += `- **Headline:** ${recHeadline}\n`;
      if (recPrimary) md += `- **Primary text:** ${recPrimary}\n`;
      if (recCta) md += `- **CTA:** ${recCta}\n`;
      md += `\n`;
    }
  }
  md += `_Generated by smOS \`/creative\`. Screened against the client's restricted-word list; final selection confirmed before \`/launch\`._\n`;
  return md;
}
...
function main() {
  ...
  const { htmlPath, pdfOk } = writeHtmlAndPdf(mdPath, md, {
    title: `${profile?.name || slug} — Ad Copy`,
    subtitle: `${out.angles.length} creative angles`,
    eyebrow: "smOS · Creative",
  });
